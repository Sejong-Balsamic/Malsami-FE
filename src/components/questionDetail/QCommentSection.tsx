...
function CommentSection({ postId, contentType, onCommentAdded }: CommentSectionProps) {
  ...
  const handleLikeClick = async (commentId: string) => {
    ...
    if (!isSameMemberById(comment.member?.memberId as string)) {
      try {
            ...
            >
              <div className="flex items-center gap-[8px]">
                {!comment.isPrivate ? (
                  <span className="font-pretendard-bold text-[14px] text-[#09bba2]">
                    @{comment.member?.uuidNickname as string}
                  </span>
                ) : (
              ...
              <div className="flex justify-end">
                <button
                  ...
                  onClick={() => handleLikeClick(comment.commentId as string)} // 좋아요 클릭 핸들러
