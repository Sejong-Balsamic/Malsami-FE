...
export default function NoticePagination({ 
  ...
  return (
    <div className="flex items-center justify-center gap-[18px]">
      {visiblePageNumbers.map((currentPageElement, elementIndex) => {
        const isEllipsisElement = currentPageElement === '...';
        
        if (isEllipsisElement) {
          return (
            <span
              key={`ellipsis-${elementIndex}`}
        ...
        return (
          <div key={actualPageNumber} className="flex flex-col items-center">
            ...
            <button
              ...
              className="font-suit text-sm font-medium leading-normal text-[#737373] hover:opacity-70 transition-opacity"
            >
              {displayPageNumber}
            </button>
            ...
            {isCurrentlyActivePage && (
              <div className="mt-0.5 w-[14px] h-px rounded-[2px] bg-[#08E4BA]" />
            )}
          </div>
        );
      })}
