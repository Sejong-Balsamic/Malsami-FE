...
import QnaSortTypeKeys from "@/lib/constants/partial/qnaSortTypeKeys";
...
const QnaFilterOptionsModal: React.FC<QnaFilterOptionsModalProps> = ({
  ...
  const [chaetakStatus, setChaetakStatus] = useState(initialFilterOptions.chaetakStatus);
  ...
  // 필터 적용 함수
  const handleApply = () => {
    onApplyFilter({ chaetakStatus, qnaPresetTags, sortType });
  };
            ...
            >
              <>
                ...
                <div className="mb-[30px] flex flex-col">
                  {/* QnaSortTypeKeys가 객체이므로 Object.keys로 key배열 생성 */}
                  {Object.keys(QnaSortTypeKeys).map(qnaSortTypeKey => (
                      ...
                      >
                        {sortType === qnaSortTypeKey ? (
                          <span className="font-pretendard-bold text-base text-custom-blue-500">
                            {SortTypes[qnaSortTypeKey as keyof typeof SortTypes]}
                            {/*qnaSortTypeKey가 SortTypes의 유효한 키인지 확인. 컴파일 단계에서 안전성을 확보하고, 런타임 오류를 방지*/}
                          </span>
                        ) : (
                ...
                <div className="mb-[30px] flex flex-col">
                  {Object.keys(ChaetaekStatus).map(chaetaekStatusKey => (
                    <li key={chaetaekStatusKey} className="flex rounded-xl py-[10px]">
                      <div
                        className="flex w-full cursor-pointer flex-row justify-between"
                        onClick={() => setChaetakStatus(chaetaekStatusKey)}
                        onKeyDown={e => e.key === "Enter" && setChaetakStatus(chaetaekStatusKey)}
                      >
                        {chaetakStatus === chaetaekStatusKey ? (
                          <span className="font-pretendard-bold text-base text-custom-blue-500">
                ...
                <div className="mb-[40px] flex flex-wrap justify-center gap-x-[7px] gap-y-[20px]">
                  {/* QnaPresetTags 객체에서 key 배열 생성 */}
                  {Object.keys(QnaPresetTags).map(qnaPresetTagKey => (
                    <button
                      key={qnaPresetTagKey}
                      onClick={() =>
                        setQnaPresetTags(
                          prevTags =>
                            prevTags.includes(qnaPresetTagKey)
                              ? prevTags.filter(t => t !== qnaPresetTagKey)
                              : [...prevTags, qnaPresetTagKey].slice(0, 2), // 태그 선택 2개만 가능하게 제한
                        )
                      }
                    ...
                    >
                      {QnaPresetTags[qnaPresetTagKey as keyof typeof QnaPresetTags]}{" "}
                      {/* 질문태그키 확인 및 한글로 렌더링 */}
                    </button>
                  ))}
                </div>
