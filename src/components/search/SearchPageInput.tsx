...
//     if (searchValue.trim()) {
//       router.push(`/search/result?query=${encodeURIComponent(searchValue)}`); // 검색어를 URL에 추가
//     }
//   };
...
//         setSearchValue(filteredTerms[activeSuggestionIndex]); // 선택된 제안으로 검색
//       }
//       routeSearchValue(); // 검색 실행
...
//               className={`font-pretendard-medium flex cursor-pointer justify-between px-6 py-2 text-base ${index === activeSuggestionIndex ? "rounded-2xl bg-gray-100" : ""}`}
//             >
//               <span> {term}</span>
