      ...
      <div className="flex justify-between">
        ...
        <button
          ...
          className={`relative w-full py-2 pb-4 ${
            activeTab === "자료게시판"
              ...
              : "text-SUIT_16 font-medium text-[#C5C5C5]"
          }`}
        ...
        >
          ...
          <span className="absolute bottom-0 left-0 h-[4px] w-full">
            {/* 회색 2px 기준선 (항상 표시) */}
            <span className="absolute left-0 top-1/2 h-[2px] w-full -translate-y-1/2 transform rounded-[2px] bg-[#E2E2E2]" />
            {/* 활성 탭 컬러 4px 바 */}
            {activeTab === "자료게시판" && (
              <span className="absolute bottom-0 left-0 h-[4px] w-full rounded-[2px] bg-document-main" />
            )}
          </span>
        </button>
        {/* 질문게시판 탭 */}
