...
import { BoardType, calculateOptimalPageSizesForAllBoards } from "@/global/store/optimalPageSizeSlice";
...
export function useOptimalPageSizeForBoard(boardType: BoardType): number {
  ...
  // Redux에서 상태 가져오기
  const { pageSizeByBoardType, isCalculationCompleted } = useSelector((state: RootState) => state.optimalPageSize);
...
export function useAllOptimalPageSizes() {
  const { pageSizeByBoardType, isCalculationCompleted, screenHeightPixels } = useSelector(
    (state: RootState) => state.optimalPageSize,
  );
