...
const optimalPageSizeSlice = createSlice({
  ...
  reducers: {
    ...
    calculateOptimalPageSizesForAllBoards: (
      state, 
      action: PayloadAction<{ screenHeightPixels: number }>
    ) => {
      ...
      const availableNoticeHeight = screenHeightPixels - HEADER_HEIGHT_PIXELS - NOTICE_PINNED_SECTION_HEIGHT - PAGINATION_SECTION_HEIGHT_PIXELS - BOTTOM_MARGIN_HEIGHT_PIXELS;
      ...
      const availableDocumentHeight = screenHeightPixels - HEADER_HEIGHT_PIXELS - PAGINATION_SECTION_HEIGHT_PIXELS - BOTTOM_MARGIN_HEIGHT_PIXELS;
      const optimalDocumentPageSize = Math.max(6, Math.min(10, Math.floor(availableDocumentHeight / DOCUMENT_CARD_HEIGHT)));

      // 질문게시판 계산
      const QUESTION_CARD_HEIGHT = 110;
      const availableQuestionHeight = screenHeightPixels - HEADER_HEIGHT_PIXELS - PAGINATION_SECTION_HEIGHT_PIXELS - BOTTOM_MARGIN_HEIGHT_PIXELS;
      const optimalQuestionPageSize = Math.max(5, Math.min(9, Math.floor(availableQuestionHeight / QUESTION_CARD_HEIGHT)));

      // 댓글 계산 (더 작은 높이)
      const COMMENT_ITEM_HEIGHT = 80;
      const availableCommentHeight = screenHeightPixels - HEADER_HEIGHT_PIXELS - PAGINATION_SECTION_HEIGHT_PIXELS - BOTTOM_MARGIN_HEIGHT_PIXELS;
      const optimalCommentPageSize = Math.max(8, Math.min(15, Math.floor(availableCommentHeight / COMMENT_ITEM_HEIGHT)));
    ...
    setPageSizeForSpecificBoard: (
      state,
      action: PayloadAction<{ boardType: BoardType; pageSize: number }>
    ) => {
