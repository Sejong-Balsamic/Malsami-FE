...
export default function NoticePage() {
  ...
  // 3. 공지사항 데이터 가져오기
  const fetchNoticePostsDataByPageNumber = async (requestedPageNumber: number = 0) => {
    try {
      ...
      if (requestedPageNumber === 0 && noticePostsApiResponse.noticePostsPage?.content && noticePostsApiResponse.noticePostsPage.content.length > 0) {
      ...
      setNoticeDataFetchErrorMessage(apiError instanceof Error ? apiError.message : "공지사항을 불러오는데 실패했습니다.");
    } finally {
    ...
    return (
      <div className="relative mx-auto w-full max-w-[640px]">
        ...
        <div className="flex h-[calc(100vh-64px)] flex-col items-center justify-center px-6">
          <p className="font-suit-medium text-lg text-foreground mb-4">오류가 발생했습니다</p>
          <p className="font-pretendard-regular text-sm text-muted-foreground text-center">{noticeDataFetchErrorMessage}</p>
        </div>
      </div>
    );
  }
  ...
  if (!currentNoticePostsPageData?.content || currentNoticePostsPageData.content.length === 0) {
    return (
      <div className="relative mx-auto w-full max-w-[640px]">
        <CommonHeader title="공지사항" rightType={RIGHT_ITEM.NONE} />
        <div className="flex h-[calc(100vh-64px)] flex-col items-center justify-center px-6">
          <p className="font-suit-medium text-lg text-foreground mb-4">공지사항이 없습니다</p>
  ...
  // 9. 메인 렌더링
  return (
    <div className="relative mx-auto w-full max-w-[640px] min-h-screen bg-background">
      <CommonHeader title="공지사항" rightType={RIGHT_ITEM.NONE} />
      ...
      <div className="px-5">
        ...
        {currentNoticePostsPageData.content.map((currentNoticePost, cardIndex) => (
          <div key={currentNoticePost.noticePostId || `notice-${cardIndex}`}>
            <NoticeCard 
              noticePost={currentNoticePost}
            />
            {/* 구분선 아래 16px 간격 */}
            {cardIndex < currentNoticePostsPageData.content!.length - 1 && (
              <div className="h-4" />
            )}
          </div>
        ))}
