...
// 접근 제한 모달 컴포넌트
function AccessRestrictedModal({ postTier, closeModal }: { postTier: PostTier; closeModal: () => void }) {
  // 표시할 티어의 대문자 변환
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative w-[330px] rounded-lg bg-white p-6 shadow-md">
        {/* 닫기 버튼 */}
        <button type="button" onClick={closeModal} className="absolute right-2 top-2 text-gray-400 hover:text-gray-600">
          ✕
        </button>
...
export default function DocumentTierPage({ params }: DocumentTierPageProps) {
  ...
  }, [memberDto, postTier, currentPage, fetchTierDocuments]);
  ...
  const handleDocumentConfirm = async (filtering: Partial<DocumentCommand>) => {
    console.error("자료 필터링 적용:", filtering);
  ...
  const handleFilterReset = () => {
    console.error("BottomSheet 내부 필터 초기화");
